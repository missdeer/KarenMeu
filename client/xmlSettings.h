...
public:
    ...
    XmlNode(const QString &name, const QString &text = QString(), QObject *parent = 0) : QObject(parent), tagName(name), subtext(text) {}
...
bool readSettingsXml(QIODevice &device, QMap<QString, QVariant> &map)
{
    ...
    while (!xml.atEnd())
    {
        ...
        case QXmlStreamReader::EndElement:
            ...
            // on the next loop iteration, we should get either the start of the next category or the closing tag of the parent (either the parent
            // category or the "parent" leaf name)
            else
